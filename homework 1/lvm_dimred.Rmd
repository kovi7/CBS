...
#     
#     p <- ggplot(PC) +
#       geom_point(aes_string(x = pc_i, y = pc_j, color = as.factor(PC$strain)), alpha = 0.8)
#     
#     print(p)
#   }
# }
...
##So I chose the V2 and V3 combination
  
pdf('Kowalska_problem2.pdf')
ggplot(PC) + geom_point(aes(x=V2, y=V3, col=as.factor(strain))) +
  ggtitle('Best combination of 2 Right Singular Vevtors')
dev.off()
...
ggplot(loadings) + 
  ...
  ggtitle('Top 2 Left Singular Vectors (Loadings)')
dev.off()
...
```{r}
tab_5 = melt(loadings[,c(1,2,3,4,5, 24)], variable.name = "loading", value.name = 'value' , id.vars = "strain")

pdf("Kowalska_problem4.pdf")
ggplot(tab_5) + 
  geom_violin(aes(x = loading, y =  value, fill = loading),draw_quantiles = c(0.25, 0.5, 0.75)) + 
  guides(fill = "none") +
  labs(title = 'Violin plots of the top 5 left singular vectors', x = "Vector (loading)", y= "Value")
dev.off()
```
...
```{r}
set.seed(1) 
kmeans_result <- kmeans(edata, centers = 5) #centers = k

tsne_result <- Rtsne(edata)
tsne_data <- as.data.table(tsne_result$Y)
tsne_data$cluster <- as.factor(kmeans_result$cluster)

pdf("Kowalska_problem5.pdf")
ggplot(tsne_data) +
  geom_point(aes(x = V1, y = V2, color = cluster)) +
  labs(title = "t-SNE Projection with K-means Clusters",
