...
pc_scores <- Embeddings(pbmc, "pca")[, 1:50]
kmeans_result <- kmeans(pc_scores, centers = 10, nstart = 25)
pbmc$kmeans10 <- as.factor(kmeans_result$cluster)
pdf("Kowalska_problem1.pdf")
DimPlot(pbmc, reduction = "tsne", group.by = "kmeans10", label = TRUE, pt.size = 0.5) + ggtitle("K-means Clusters (k=10)")
dev.off()
```
...
pbmc$hierarchical_cluster <- as.character(pbmc$kmeans10) 
num_main_clusters <- length(levels(pbmc$kmeans10))
subcluster_plots <- list() 

find_optimal_k <- function(data) {
  silhouette_width <- sapply(2:5, function(k) {
  km <- kmeans(data, k, nstart = 25)
  ss <- silhouette(km$cluster, dist(data))
  mean(ss[,3])
  })
  which.max(silhouette_width) + 1
}

for (i in levels(pbmc$kmeans10)) {
  cluster_label <- paste("Cluster", i)
  cells_in_cluster <- WhichCells(pbmc, expression = kmeans10 == i)
  pcs_subset <- pc_scores[cells_in_cluster, ]

  optimal_k <- find_optimal_k(pcs_subset)
  sub_k <- optimal_k
  sub_kmeans_result <- kmeans(pcs_subset, centers = sub_k, nstart = 25)

  pbmc$hierarchical_cluster[cells_in_cluster] <- paste0(i, ".", sub_kmeans_result$cluster)
  p <- DimPlot(pbmc,
               reduction = "tsne",
               ...
               label = FALSE, 
               pt.size = 0.5) +
    ggtitle(paste("Subclusters for cluster", i, "(k=", sub_k, ")")) +
    NoLegend() +

    coord_cartesian(xlim = range(Embeddings(pbmc, "tsne")[,1]),
                    ylim = range(Embeddings(pbmc, "tsne")[,2]))
  print(p)
  subcluster_plots[[cluster_label]] <- p
}

pbmc$hierarchical_cluster <- factor(pbmc$hierarchical_cluster)

overall_plot <- DimPlot(pbmc,
                        reduction = "tsne",
                        group.by = "hierarchical_cluster",
                        label = TRUE,
                        repel = TRUE,
                        label.size = 2.5,
                        pt.size = 0.5) +
  NoLegend() +  ggtitle("Overall visualization")


pdf("Kowalska_problem2.pdf", width = 12, height = 8)

print(overall_plot)

for(cluster_name in sort(names(subcluster_plots))) {
  print(subcluster_plots[[cluster_name]])
}
